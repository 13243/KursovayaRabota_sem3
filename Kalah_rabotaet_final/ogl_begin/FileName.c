...
enum difficultyLevel {
	...
	MEDIUM = currentlyTestedlevel,
	HARD = currentlyTestedlevel
};
...
int miniMaxAB(int* kalahBoard, int curDepth, enum SIDE side, int alpha, int beta) {
	...
	if (side == B) {
		...
		for (move = 12; move > 6; move--) { // Iterate over Player B's moves (pits 7 to 12)
			memcpy(simulatedkalahBoard, kalahBoard, 14 * sizeof(int));
...
void displayButton(Button button)
{
	...
	glColor3f(0.2, 0.5, 0.5); // Set the color for the button (green)

	// Define the vertex pointer and draw the button as a filled quadrilateral
	glVertexPointer(2, GL_FLOAT, 0, button.vertices); // Specify the vertex data
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	while (!bQuit)  // Main message loop
	{
		...
		else
		{
			...
			int boardCounter = 0;  // Index for looping
			...
			for (boardCounter = 0; boardCounter < 14; boardCounter++) {
				if (boardCounter < 7 || (boardCounter > 7 && boardCounter > 12)) {
					kalahBoard[count + 1] = '0' + initialkalahBoard[boardCounter] % 10;
		...
		if (currentPlayerResponse == 1) {
			...
			for (int i = 0; i < 12; i++) {
				...
				if (isPointInButton(LOWORD(lParam), HIWORD(lParam), gameButtons[i])) {
					...
					if (((strcmp(gameButtons[i].buttonName, "B1") == 0) ||
						...
						int boardIndex = 14 - atoi(&gameButtons[i].buttonName[1]);
			...
			for (int i = 0; i < 4; i++) {
				...
				if (isPointInButton(LOWORD(lParam), HIWORD(lParam), buttons[i])) {
					...
					if (currentPlayerResponse == 0) {
						if (strcmp(buttons[i].buttonName, "quit") == 0) {
							// If the "quit" button is clicked and it's the initial state, exit the application
							PostQuitMessage(0);
						}
						else if (strcmp(buttons[i].buttonName, "players") == 0) {
							// Switch to player vs player mode
							currentPlayerResponse = 1;
						}
						else if (strcmp(buttons[i].buttonName, "computer") == 0) {
							// Switch to player vs computer mode
							currentPlayerResponse = 2;
						}
					}
					else if (currentPlayerResponse == 2) {
						int difficulty = -1; // Default: no difficulty set
						if (strcmp(buttons[i].buttonName, "quit") == 0) {
							difficulty = EASY;
						}
						else if (strcmp(buttons[i].buttonName, "players") == 0) {
